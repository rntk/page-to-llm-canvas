...
function makeLeaf(fullPath, sentences = []) {
  return {
    node: { name: fullPath.split(">").pop(), fullPath, depth: fullPath.split(">").length - 1, topic: { sentences } },
    children: new Map(),
    parent: null,
  };
}
...
function makeBranch(fullPath, childEntries = []) {
  ...
  return {
    node: { name: fullPath.split(">").pop(), fullPath, depth: fullPath.split(">").length - 1, topic: null },
    children,
    parent: null,
  };
}
