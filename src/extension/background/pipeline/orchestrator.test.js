...
import { wrapCallLLMWithRetry } from '../../../core/metrics/llm.js';
import * as parserMetrics from '../../../core/metrics/parser.js';
import * as resplitMetrics from '../../../core/metrics/resplit.js';
...
import { TOPIC_RANGE_INPUT_MAX_SENTENCES } from '../../../core/pipeline/pipelineConfig.js';
import { TOPIC_RANGE_MAX_SENTENCES } from '../../../core/pipeline/topicRangeResplit.js';
...
vi.mock('../../../core/metrics/parser.js', () => ({
  recordParserMetric: vi.fn(async () => undefined),
}));

vi.mock('../../../core/metrics/resplit.js', async () => {
  const actual = await vi.importActual('../../../core/metrics/resplit.js');
  return {
    ...actual,
...
describe('runPipeline', () => {
  ...
  it('skips verbose processing-log stages when the setting is off', async () => {
    ...
    expect(stages).not.toContain('normalizing_text_start');
  ...
  it('records verbose processing-log stages when the setting is on', async () => {
    ...
    const stages = loggedStages();
    expect(stages).toContain('pipeline_start');
    expect(stages).toContain('normalizing_text_start');
  ...
  it('clears stale topics and summaries on a fresh run (non-resume path)', async () => {
    ...
    expect(firstUpdateCall[1]).toEqual({
      status: 'splitting',
      progress: { stage: 'normalizing_text', done: 0, total: 0 },
      error: null,
      topics: [],
      topic_summaries: {},
      topic_summary_index: {},
